...
import LeftMarker from './LeftMarker';
...
export class MultiSlider extends React.Component {
  ...
  render() {
    ...
    const markerContainerOne = { top: markerOffsetY - 24, left : trackOneLength + markerOffsetX - 42 };
    const markerContainerTwo = { top: markerOffsetY - 24, right: trackThreeLength + markerOffsetX - 42 };

    let renderTimes = [];
    for (let i = 0; i < this.props.max - this.props.min + 1; i++) {
      let key = "renderTimesKey" + i;
      let str = "";
      if (this.props.startTime + this.props.max - this.props.min - i > 23) {
        str = this.props.startTime + this.props.max - this.props.min - i - 24;
      } else {
        str = this.props.startTime + this.props.max - this.props.min - i;
      }
      if (str < 10) {
        str = "0" + str;
      }
      if (i === this.state.valueOne || i === this.state.valueTwo) {
        renderTimes.push(
    ...
    return (
      <View style={{ width: Dimensions.get('window').width, height: 45 }}>
        <CardView
          style={{
            position: 'absolute',
        ...
        <View style={[Platform.OS === 'ios' ? styles.containerIOS : styles.container, this.props.containerStyle, {justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <View style={{
          width: (Dimensions.get('window').width - 100) / 10 * 12,
          height: 50,
          flexDirection: 'row',
          marginLeft: (Dimensions.get('window').width - (Dimensions.get('window').width - 100) / 10 * 12) / 2
        }}>
          {
            renderTimes
          }
